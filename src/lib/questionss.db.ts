...
export class QuestionsDbClient implements IQuestions {
  ...
  async getQuestions(
    ...
    let totalQuestions;
    try {
      totalQuestions = await this.prisma.questions.count({
        where: {
          category: {
            slug: categorySlug,
          },
  ...
  async createQuestion(
    ...
    let savedQuestion;
    try {
      savedQuestion = await this.prisma.questions.create({
        data: {
  ...
  async updateQuestion(
    ...
    let updatedQuestion;
    try {
      updatedQuestion = await this.prisma.questions.update({
        where: {
          id,
        },
        data: {
  ...
  async deleteQuestion(id: Id): Promise<Result<boolean | null>> {
    let questionExists;
    try {
      questionExists = await this.prisma.questions.findFirst({
        where: {
          id,
        },
      });
