...
export class CategoriesDbClient implements ICategory {
  ...
  async getCategories(
    ...
    let totalCategories;
    try {
      totalCategories = await this.prisma.categories.count();
    } catch (error) {
  ...
  async createCategory(
    ...
    try {
      ...
      categoryExists = await this.prisma.categories.findFirst({
        where: {
    ...
    try {
      ...
      savedCategory = await this.prisma.categories.create({
        data: {
          title: category.title,
          slug,
        },
  ...
  async updateCategory(
    ...
    const newSlug = slugify(category.title);
